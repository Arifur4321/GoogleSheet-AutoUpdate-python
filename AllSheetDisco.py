...
def fetch_close_data(email, start_date, end_date):
    ...
    if not user_ids:
        ...
        return 0, 0, 0  # Return an additional value for leads.created.all.count
        ...
        if len(row) > 2 and row[2]:  # Identify rows with email addresses
            ...
            for i in range(row_index, len(sheet_data)):
                if len(sheet_data[i]) > 0:
                    if sheet_data[i][0].strip().lower() == "call":
                        call_row = i + 1
                    if sheet_data[i][0].strip().lower() == "duration":
                        duration_row = i + 1
                    if sheet_data[i][0].strip().lower() == "disco":
                        disco_row = i + 1
                if call_row and duration_row and disco_row:
                    break

            if not call_row or not duration_row or not disco_row:
                logging.warning(f"Could not locate 'Call', 'Duration', or 'Disco' rows for email: {email} in sheet: {sheet_name}")
                continue
            ...
            for col_index, date in enumerate(headers[3:], start=3):  # Dates start from column D
                ...
                if date:
                    try:
                        ...
                        total_calls, total_duration, leads_created = fetch_close_data(email, formatted_date, formatted_date)
                        ...
                        # Update the "Disco" row
                        disco_cell_range = f"{sheet_name}!{column_letter}{disco_row}"
                        update_google_sheet(service, sheet, disco_cell_range, leads_created)
                        ...
                        logging.info(f"Updated {disco_cell_range} with {leads_created}")
