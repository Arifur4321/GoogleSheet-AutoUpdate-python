...
def fetch_discovery_programmata_data(assignee_name, date_filter, discovery_filter):
    ...
    skip = 0
    all_leads = []
    matching_lead_names = set()  # Use a set for faster lookups
    ...
    for lead in all_leads:
        for opportunity in lead.get('opportunities', []):
            ...
            if (
                ...
                matching_lead_names.add(lead_name)

    print("Matching lead count -----------------------:", len(matching_lead_names))
    print("Matching lead names -----------------------:", matching_lead_names)
    return len(matching_lead_names)
